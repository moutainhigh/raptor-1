...
public interface AuditOperationRecordRepository extends JpaRepository<AuditOperationRecordEntity, Long> {
    ...
    @Query(value = "select  all_audit_num ,operate_id,login_name,name,ifnull(num2,0) AS manual_audit_num FROM (select count(*) all_audit_num ,operate_id,status,login_name,name from t_audit_operation_record r  LEFT JOIN t_audit_user u on u.id = r.operate_id where distribute_id = ?1 GROUP BY operate_id) AS a " +
            "LEFT JOIN " +
            "(select count(*) num2 ,operate_id o_id from t_audit_operation_record r  LEFT JOIN t_audit_user u on u.id = r.operate_id where `status`= 'MANUAL' and distribute_id = ?1 GROUP BY operate_id) b " +
            "ON b.o_id = a.operate_id;", nativeQuery = true)
