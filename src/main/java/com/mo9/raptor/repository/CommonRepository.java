...
public interface CommonRepository extends JpaRepository<DictDataEntity,Long> {
    ...
    @Query(value = "select \n" +
            ...
            "(select count(*)  from t_raptor_loan_order where `status` = 'LENT' and DATE(FROM_UNIXTIME(repayment_date/1000)) < DATE(FROM_UNIXTIME(?1))  ) as 'overdueNumber'", nativeQuery = true)
