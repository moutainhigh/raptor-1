...
public interface LendOrderRepository extends JpaRepository<LendOrderEntity,Long>, JpaSpecificationExecutor<LendOrderEntity> {
     ...
     * @return
     */
    @Query(value = "select sum(apply_number) as 'dailyLendAmount' from t_raptor_lend_order where status in ('PENDING', 'LENDING', 'SUCCESS') AND create_time >= ?1 and deleted = false", nativeQuery = true)
