package com.mo9.raptor.utils.log;

/**
 * Created by ycheng on 2018/8/1.
 */
...
public class Log {
    ...
    @Autowired
    private DingTalkNoticeThreadTask dingTalkNoticeThreadTask;
    ...
    public void init() {
        logStatic = this;
        logStatic.threadPool = this.threadPool;
        logStatic.dingTalkNoticeThreadTask = this.dingTalkNoticeThreadTask;
    ...
    public static void error(Logger log, Throwable e, String format, Object... arguments) {
        String content = format(format, arguments);
        log.error(content, e);

        logStatic.dingTalkNoticeThreadTask.setTitle(content);
        logStatic.dingTalkNoticeThreadTask.setMessage(ExceptionUtils.getStackTrace(e));
        logStatic.threadPool.execute(logStatic.dingTalkNoticeThreadTask);
