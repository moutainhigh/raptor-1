...
public interface RiskThirdBlackListRepository extends JpaRepository<TRiskThirdBlackList, Long> {
    ...
    @Query(value = "SELECT count(*)>0 from t_risk_third_black_list WHERE BLACK_TYPE in ('mobile','idcard')  and BLACK_TYPE IS NOT NULL and BLACK_VALUE = ?1", nativeQuery = true)
